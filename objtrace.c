...
static void objtrace_dump_zval(zval *o) {
    switch (Z_TYPE_P(o)) {
        ...
        case IS_OBJECT: {
        	...
        	//zend_hash_apply_with_argument(ht, yara_hash_apply_helper, o);
            fprintf(OTG(log)," Object [%s] ", ce->name);
        	ht = o->value.obj.handlers->get_properties(o TSRMLS_CC);
        	p = ht->pListHead;
        	while (p != NULL) {
        		fprintf(OTG(log), " %s:", p->arKey);
        		//yara_dump_zval(p->pData);
        		//v = o->value.obj.handlers->read_property(o, p->pData, 0);
        		//yara_dump_zval(v);
...
PHP_MINIT_FUNCTION(objtrace) {
	...
	zend_hash_init_ex(OTG(sources), 50, NULL, (dtor_func_t)objtrace_source_dtor, 1, 0);
